...
plot_variant_overlay <- function(gene,
    ...
    if (variant_pos < 1L || variant_pos > protein_length) {
        stop(sprintf(
            "variant_pos %d outside protein (length %d)",
    ...
    if (!(ref_name %in% seq_names)) {
        stop(sprintf(
            "ref_name '%s' not found in FASTA. Sequences: %s",
    ...
    p_msa_data <- ggplot2::ggplot(df_msa_window, ggplot2::aes(x = msa_col, y = seq_name)) +
        ...
        ggplot2::coord_cartesian(
            xlim = c(start_col - 0.5, end_col + 0.5),
    ...
    if (length(display_codes) > 0L) {
        ...
        p_acmg_data <- ggplot2::ggplot(chip_df) +
            ...
            ggplot2::scale_fill_manual(
                values = stats::setNames(chip_df$fill, chip_df$code),
                guide = "none"
            ) +
    ...
    make_track_panel <- function(df, value_col, track_title, custom_scale) {
        if (is.null(df) || nrow(df) == 0L) {
            return(list(label = NULL, data = NULL))
        }
    ...
    if (!is.null(domains_df) && nrow(domains_df) > 0L) {
        ...
        p_dom_data <- ggplot2::ggplot(visible_doms) +
            ...
            ggplot2::annotate("text",
                x = text_center, y = 1.0, label = DOMAIN_DISPLAY_LABEL,
    ...
    cv_pair <- make_track_panel(
        cv, "significance", "ClinVar",
        ggplot2::scale_fill_manual(
            values = cv_palette,
            name = "ClinVar"
        )
    ...
    if (!is.null(gnomad_df) && "af_joint" %in% names(gnomad_df)) {
        ...
        gnomad_pair <- make_track_panel(
            gnomad_agg, "af_log", "gnomAD",
    ...
    if (!is.null(cadd_df)) {
        ...
        cadd_pair <- make_track_panel(
            cadd_agg, "cadd_phred", "CADD",
            ggplot2::scale_fill_gradientn(
                colors = col$cadd,
    ...
    if (!is.null(am)) {
        ...
        am_pair <- make_track_panel(
            am_agg, "am_score", "AlphaMissense",
            ggplot2::scale_fill_gradientn(
    ...
    if (!is.null(revel_df)) {
        ...
        revel_pair <- make_track_panel(
            revel_agg, "revel_score", "REVEL",
            ggplot2::scale_fill_gradientn(
                colors = col$revel,
    ...
    ## ---------------------------------------------------------------
    track_rows <- list()
    add_track <- function(rows_acc, active, label, data, key) {
        if (isTRUE(active) && !is.null(data)) {
            rows_acc[[length(rows_acc) + 1L]] <-
                list(key = key, label = label, data = data)
        }
        rows_acc
    }
    track_rows <- add_track(track_rows, show_domain, p_dom_label, p_dom_data, "domain")
    track_rows <- add_track(track_rows, show_clinvar, cv_pair$label, cv_pair$data, "clinvar")
    track_rows <- add_track(track_rows, show_gnomad, gnomad_pair$label, gnomad_pair$data, "gnomad")
    track_rows <- add_track(track_rows, show_cadd, cadd_pair$label, cadd_pair$data, "cadd")
    track_rows <- add_track(track_rows, show_alphamissense, am_pair$label, am_pair$data, "alphamissense")
    track_rows <- add_track(track_rows, show_revel, revel_pair$label, revel_pair$data, "revel")
    ...
    add_axis <- function(p) {
        ...
        keep <- !is.na(tick_cols)
        pretty_pos <- pretty_pos[keep]
        tick_cols <- tick_cols[keep]
        p +
    ...
    rows <- list()
    add_row <- function(rows_acc, label, data, height) {
        rows_acc[[length(rows_acc) + 1L]] <- list(label = label, data = data, height = height)
        rows_acc
    }

    rows <- add_row(rows, p_variant_label, p_variant_data, 0.7)
    if (isTRUE(show_acmg)) rows <- add_row(rows, p_acmg_label, p_acmg_data, 0.55)
    rows <- add_row(rows, p_msa_labels, p_msa_data, 4.5)
    for (tr in track_rows) rows <- add_row(rows, tr$label, tr$data, 0.5)
    if (have_legends) rows <- add_row(rows, p_panel3_label, p_panel3_data, 1.8)
    ...
    if (is.null(title)) {
        title <- sprintf(
            "%s %s: Clinical-Genetics Annotation Overlay Stack",
            gene, VARIANT_LABEL
        )
    }
    if (is.null(subtitle)) {
        subtitle <- sprintf(
            "%d-species MSA window, residues %d..%d",
