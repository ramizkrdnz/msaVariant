    ...
    for (col in intersect(names(schema), names(df))) {
        ...
        if (expected == "factor" && is.factor(df[[col]]) &&
            ...
            if (!identical(levels(df[[col]]), factor_levels[[col]])) {
                errs <- c(errs, sprintf(
                    "%s$%s: factor levels mismatch (got: %s)",
                    table_name, col,
...
validate_gene_data <- function(obj, strict = FALSE) {
    ...
    if ("meta" %in% names(obj)) {
        ...
        if (is.data.frame(obj$meta) && nrow(obj$meta) != 1L) {
            errs <- c(errs, sprintf(
                "meta: must have exactly 1 row (has %d).",
                nrow(obj$meta)
            ))
        }
    }
    ...
    if (length(errs) == 0L) {
        ...
        if (strict) {
            stop(paste0(
                "validate_gene_data() failed validation:\n  ",
                paste(errs, collapse = "\n  ")
            ), call. = FALSE)
        }
