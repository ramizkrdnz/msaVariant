...
# The default PDF device works everywhere; cairo_pdf (below) is an
# optional upgrade for better font embedding where it is available.
ggplot2::ggsave("demo1_R21H.pdf", p, width = 15, height = 9)

# Optional: nicer font embedding, when your R has a working cairo
# (stock Windows/Linux R; on macOS it needs XQuartz). Falls back
# gracefully — just use the line above if cairo is unavailable.
...
at any zoom, and a copy-editor can select the text. The default
`ggsave(".pdf")` device produces this on every platform; `cairo_pdf` is
only an optional refinement for font embedding and needs a cairo-capable
R (on macOS, that means XQuartz is installed).
